...
public class CompositeRequirementComponent extends CompositeAnnotatorSuper {

	private final FormToolkit toolkit = new FormToolkit(Display.getCurrent());
	private DefaultTableModel modelTable;
	private JTable table;
	private String[] columnNames = { "Detail", "Label", "Content", "End date", "Start date" };
	private Frame frame;
	 ...
	 * @param style
	 */
	public CompositeRequirementComponent(Composite parent, int style,int type) {
		super(parent, style);
	...
	private DefaultTableModel getModelTable(List<List> listOfLists) {
		...
		for (int loopIndex = 0; loopIndex < listOfLists.size(); loopIndex++) {
			...
			model.addRow(new Object[] { Boolean.FALSE,listItems.get(0).toString(),content, listItems.get(3).toString(),listItems.get(2).toString(), listItems.get(3).toString(),listItems.get(1).toString()});
		}
