...
public class BKASTVisitor extends ASTVisitor {
	...
	private void genClassMetric(TypeDeclaration type)
    {
	   ...
	   */
	   ...
	   public boolean visitSemanticComment(Javadoc javadoc, String idSemanticComment){
		   InstanceData instanceSemanticComment = new InstanceData();
		   instanceSemanticComment.setClassName(ConsistentOntology.SEMANTIC_COMMENT);
		   instanceSemanticComment.setInstanceID(idSemanticComment);
		   instanceSemanticComment.setInstanceLabel("Semantic Comment");
		   InitInstance initSemanticCommentInstance = new InitInstance(instanceSemanticComment);
		   String description =removeAllTagInComment(javadoc.toString());
		   description = description.replace("*", "");
		   description = description.replace("/", "");
		   description = description.replace("\\", "");
		   if((description!=null)&&(description!=""))
		   initSemanticCommentInstance.addDataProperty(ConsistentOntology.DESCRIPTION,description);
			   ...
			   TagElement newtags = (TagElement) commenttags.get(k);
				...
				if (newtags.getTagName() != null) {
					if (newtags.getTagName().equals("@model")) {
						...
						//check ?
						for (int j = 0; j < array_tags.size(); j++) {
							initSemanticCommentInstance.addDataProperty(ConsistentOntology.SEMANTIC_MODEL, array_tags.get(j).toString());
						}
					}
					if (newtags.getTagName().equals("@topic")) {
						...
						//check ?
						for (int j = 0; j < array_tags.size(); j++) {
							initSemanticCommentInstance.addDataProperty(ConsistentOntology.SEMANTIC_TOPIC, array_tags.get(j).toString());
						}
					}
					if (newtags.getTagName().equals("@function")) {
						...
						//check ?
						for (int j = 0; j < array_tags.size(); j++) {
							initSemanticCommentInstance.addDataProperty(ConsistentOntology.SEMANTIC_FUNCTION, array_tags.get(j).toString());
						}
					}
				}
		   }

		   listofAnnotation.add(initSemanticCommentInstance.getPackageField());
		   return true;
	   }
