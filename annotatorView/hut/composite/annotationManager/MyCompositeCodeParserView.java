...
public class MyCompositeCodeParserView extends Composite implements IObjectExecuteJob{
	...
	public MyCompositeCodeParserView(Composite parent, int style, TreeViewer viewer) {
		super(parent, style);
	...
	public void addProject(String projectName) {
		...
		MyASTParseProject ast = new MyASTParseProject(projectName);
		ast.run();
	...
	{	
		...
		job.setObjectExecuteJob(MyCompositeCodeParserView.this);
		job.setUser(true);
		job.schedule();
	}
		...
		PlatformUI.getWorkbench().getDisplay().syncExec(new Runnable() {
			public void run() {
				try {
					...
					if (sel.isEmpty())
					{
						MessageDialog.openError(parseAgainButton.getShell(), "Error", "Choose a code component first!");
						return;
					}
					obj = ((TreeObject) sel.getFirstElement());
//					TreeNodeData node = (TreeNodeData) obj.getData();
//					System.out.println("PROJECT NAME: " +node.getName());
//					addProject(node.getName());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//		addProject(node.getName());
	...
	private void messageError(final String title, final String message)
	{
		PlatformUI.getWorkbench().getDisplay().syncExec(new Runnable() {
			public void run() {
				try {	
					MessageDialog.openError(parseAgainButton.getShell(), title, message);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
