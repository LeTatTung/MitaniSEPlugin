...
import hut.composite.annotationManager.MyCompositeCodeParserView;
...
public class SeCommentManager extends SeSuperComposite {
	...
	private MyCompositeCodeParserView parserView;
	...
	public SeCommentManager(Composite parent, int style) {
		...
		final Composite compositeSourceTree = new Composite(sashForm_1, SWT.BORDER);
		compositeSourceTree.setLayout(new BorderLayout(0, 0));
		toolkit.adapt(compositeSourceTree);

		final Composite compositeLableSource = new Composite(compositeSourceTree, SWT.BORDER);
		...
		final CLabel methodsInClassLabel = new CLabel(sashForm_2, SWT.NONE);
		...
		final Composite compositeTree = new Composite(compositeSourceTree, SWT.BORDER);
		...
		parserView = new MyCompositeCodeParserView(sashForm_2, SWT.NULL, treeViewer);
//		parserView.addProject("Test1");
		...
		final Composite compositeContentComment = new Composite(composite_right, SWT.NONE);
		compositeContentComment.setLayout(new BorderLayout(0, 0));
		toolkit.adapt(compositeContentComment);

		final Composite compositeLableComment = new Composite(compositeContentComment, SWT.BORDER);
		...
		final CLabel methodCallFlowLabel = new CLabel(compositeLableComment, SWT.NONE);
	...
	public void setController() {
		...
		this.setController(seControllerArtifactViewPart);
		seCommentContent.setController(seControllerArtifactViewPart);
	}
